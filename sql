...
-- Products: add hsn_code and company if missing
ALTER TABLE products
  ADD COLUMN IF NOT EXISTS hsn_code VARCHAR(20) DEFAULT '' AFTER gst_rate,
  ADD COLUMN IF NOT EXISTS company  VARCHAR(100) DEFAULT '' AFTER category;

-- Sales: add narration if missing
ALTER TABLE sales
  ADD COLUMN IF NOT EXISTS narration TEXT AFTER payment_status;

-- Sale items: add gst_rate, hsn_code, and item-level discount if missing
ALTER TABLE sale_items
  ADD COLUMN IF NOT EXISTS gst_rate  DECIMAL(5,2) DEFAULT 0.00 AFTER total_price,
  ADD COLUMN IF NOT EXISTS hsn_code  VARCHAR(20)  DEFAULT ''   AFTER gst_rate,
  ADD COLUMN IF NOT EXISTS discount  DECIMAL(5,2) DEFAULT 0.00 AFTER hsn_code;

-- Purchases: add narration if missing
ALTER TABLE purchases
  ADD COLUMN IF NOT EXISTS narration TEXT AFTER payment_status;

-- Purchases: add subtotal and discount if missing (bill-level discount support)
ALTER TABLE purchases
  ADD COLUMN IF NOT EXISTS subtotal DECIMAL(10,2) DEFAULT 0.00 AFTER bill_no,
  ADD COLUMN IF NOT EXISTS discount DECIMAL(5,2)  DEFAULT 0.00 AFTER subtotal;

-- Purchase items: add gst_rate, hsn_code, and item-level discount if missing
