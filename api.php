...
switch ($table) {
    ...
    case 'products':
        ...
        if ($method === 'POST') {
            ...
            $st = db()->prepare(
                "INSERT INTO products (name,category,company,buy_price,sell_price,stock,min_stock,unit,barcode,gst_rate,hsn_code)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?)"
            );
            $st->execute([
                $d['name'], $d['category'] ?? 'Other', $d['company'] ?? '',
        ...
        if ($method === 'PUT' && $id) {
            ...
            $st = db()->prepare(
                "UPDATE products SET name=?,category=?,company=?,buy_price=?,sell_price=?,stock=?,min_stock=?,unit=?,barcode=?,gst_rate=?,hsn_code=?
                 WHERE id=?"
            );
            $st->execute([
                $d['name'], $d['category'] ?? 'Other', $d['company'] ?? '',
        ...
        if ($method === 'POST') {
            ...
            try {
                ...
                foreach (($d['items'] ?? []) as $item) {
                    $si = db()->prepare(
                        "INSERT INTO sale_items (sale_id,product_id,product_name,quantity,unit_price,total_price,gst_rate,hsn_code,discount)
                         VALUES (?,?,?,?,?,?,?,?,?)"
                    );
                    $hsn = $item['hsn_code'] ?? '';
                    ...
                    $si->execute([
                        ...
                        $item['gst_rate'] ?? 0, $hsn, $item['discount'] ?? 0
                    ]);
    ...
    case 'purchases':
        ...
        if ($method === 'POST') {
            ...
            try {
                $st = db()->prepare(
                    "INSERT INTO purchases (supplier_name,bill_no,subtotal,discount,total,payment_status,purchase_date,narration)
                     VALUES (?,?,?,?,?,?,?,?)"
                );
                $st->execute([
                    $d['supplier_name'] ?? '', $d['bill_no'] ?? '',
                    $d['subtotal'] ?? ($d['total'] ?? 0), $d['discount'] ?? 0,
                ...
                foreach (($d['items'] ?? []) as $item) {
                    $si = db()->prepare(
                        "INSERT INTO purchase_items (purchase_id,product_id,product_name,quantity,unit_price,total_price,gst_rate,hsn_code,discount)
                         VALUES (?,?,?,?,?,?,?,?,?)"
                    );
        ...
        if ($method === 'PUT' && $id) {
            ...
            try {
                ...
                $st = db()->prepare("UPDATE purchases SET supplier_name=?,bill_no=?,purchase_date=?,subtotal=?,discount=?,total=?,payment_status=?,narration=? WHERE id=?");
                $st->execute([$d['supplier_name']??'',$d['bill_no']??'',$d['purchase_date']??date('Y-m-d'),$d['subtotal']??($d['total']??0),$d['discount']??0,$d['total']??0,$d['payment_status']??'paid',$d['narration']??'',$id]);
                db()->commit();
                resp(200, ['success' => true]);
            } catch (Exception $e) {
                db()->rollBack();
